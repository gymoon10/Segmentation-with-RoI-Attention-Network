...
class Decoder(nn.Module):
    ...
    def __init__(self, num_classes=3):
        ...
        self.layers2 = nn.ModuleList()
        self.layers2.append(UpsamplerBlock(64, 32))
        self.layers2.append(non_bottleneck_1d(32, 0, 1))
        self.layers2.append(non_bottleneck_1d(32, 0, 1))

        self.layers3 = nn.ModuleList()
        self.layers3.append(UpsamplerBlock(32, 16))
        self.layers3.append(non_bottleneck_1d(16, 0, 1))
        self.layers3.append(non_bottleneck_1d(16, 0, 1))

        self.emb_conv = nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1, bias=True)

        self.output_conv = nn.Conv2d(16, num_classes, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, input):
        ...
        for layer in self.layers2:
            output = layer(output)  # (N, 32, 256, 256)

        for layer in self.layers3:
            output = layer(output)  # (N, 16, 512, 512)

        feat = self.emb_conv(output)  # (N, 32, 512, 512)

        output = self.output_conv(output)  # (N, num_classes, 512, 512)
...
class AuxDecoder(nn.Module):
    ...
    def __init__(self, num_classes=3):
        ...
        self.out_conv = nn.Conv2d(32, num_classes, kernel_size=3, stride=1, padding=1, bias=True)

    def forward(self, feat_image, feat_roi):

        output = feat_image + feat_roi  # (N, 32, 512, 512)

        output = self.conv1(output)
        output = self.out_conv(output)  # (N, num_classes, 512, 512)
